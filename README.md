...
| [SECURITY_TESTING.md](SECURITY_TESTING.md) | **วิธีการทดสอบความปลอดภัย ผลการทดสอบ และช่องโหว่ที่พบ** |
...
- **ประเมินทักษะการเขียน** ที่ `/writing` — ได้ระดับที่ประเมินได้ จุดที่ทำได้ดี และประโยคที่ผิดพร้อมฉบับแก้แล้วทีละจุด เป็นสิ่งเดียวที่ข้อสอบปรนัยวัดไม่ได้ เพราะปรนัยวัดแค่ว่าเลือกข้อถูกได้ไหม ไม่ได้วัดว่าเขียนเองได้ไหม
- **ผู้ช่วยตอบคำถาม** มุมขวาล่างของทุกหน้า ถามเรื่องการสอบ คอร์ส และการชำระเงินได้ ตอบจากข้อมูลจริงในฐานข้อมูลก่อน จึงบอกราคาและสถานะที่ถูกต้องเสมอ
...
- บันทึกการเรียกใช้ AI ทุกครั้งที่ `/admin/ai` — จำนวนครั้ง สถานะ โทเคน และเวลาตอบสนอง ทำให้ "AI ทำงานจริงไหม" เป็นตัวเลขที่ตรวจสอบได้
...
SETTINGS_SECRET="ข้อความสุ่มยาว ๆ"   # กุญแจเข้ารหัส API key ที่เก็บในฐานข้อมูล
OPENAI_API_KEY="sk-..."             # ไม่บังคับ กรอกผ่านหน้าตั้งค่าของผู้ดูแลได้
```

> ⚠️ `.env` และ `.env.production` อยู่ใน `.gitignore` แล้ว ห้าม commit ไฟล์เหล่านี้เข้า git
> Prisma อ่านจาก `.env` ส่วน Next.js อ่าน `.env.production` ตอน build — ถ้าใช้ทั้งสองไฟล์ต้องใส่ `DATABASE_URL` ให้ตรงกัน
...
**AI และผู้ช่วยตอบคำถาม**
| Method | Endpoint | หน้าที่ |
|---|---|---|
| POST | `/api/chat` | ผู้ช่วยตอบคำถาม คืน `source` และ `usedAI` มาด้วยเพื่อบอกว่าคำตอบมาจากชั้นไหน |
| GET | `/api/ai/writing?subject=` | โจทย์เขียนที่ตรงกับระดับปัจจุบัน |
| POST | `/api/ai/writing` | ตรวจงานเขียน คืนระดับ จุดแข็ง และประโยคที่แก้แล้ว |
| POST | `/api/ai/explain` | อธิบายว่าตอบผิดเพราะเข้าใจอะไรคลาดเคลื่อน |

**ผู้ดูแล**
| Method | Endpoint | หน้าที่ |
|---|---|---|
| GET | `/api/admin/students` | ผู้เรียนทุกคนพร้อมระดับรายวิชาและการชำระเงิน |
| GET | `/api/admin/ai` | บันทึกการเรียกใช้ AI ทุกครั้ง |
...
1. การเข้ารหัสรหัสผ่านใช้ salt ค่าคงที่และจำนวนรอบต่ำกว่ามาตรฐานปัจจุบัน (ดู [SECURITY_TESTING.md](SECURITY_TESTING.md) หัวข้อ 4.1)
2. เซสชันยกเลิกจากฝั่งเซิร์ฟเวอร์ไม่ได้
3. รหัสยืนยันการสมัครผู้ดูแลมีค่าเริ่มต้นในซอร์สโค้ด — ผู้ติดตั้งต้องเปลี่ยนเอง
4. ยังไม่มีระบบรีเซ็ตรหัสผ่านด้วยตัวเอง
5. ยังไม่มีชุดทดสอบหน่วยของตรรกะการสอบ (มีชุดทดสอบความปลอดภัยแล้ว)
